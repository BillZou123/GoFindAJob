...
from backend.job_qa_agent.agent import root_agent as job_qa_agent
...
def call_job_qa_agent(user_id: str, prompt: str, user_context: dict = None) -> str:
    """
    Call the job Q&A agent with the given prompt and user context.
    ...
    logger.info("=== CALL_JOB_QA_AGENT START ===")
    logger.debug(f"User ID: {user_id}")
    logger.debug(f"Prompt length: {len(prompt)}")
    logger.debug(f"User context: {user_context}")
    ...
    # Create runner with the job Q&A agent
    logger.info("Creating runner with job_qa_agent...")
    runner = Runner(
        agent=job_qa_agent,
        app_name=APP_NAME,
        session_service=session_service,
    ...
    logger.info(f"Running job Q&A agent with session {SESSION_ID}...")
    ...
    try:
        for event in runner.run(
            ...
            if event.is_final_response():
                ...
                if event.content and event.content.parts:
                    response_text = event.content.parts[0].text
                    logger.info(f"Job Q&A agent response: {response_text[:100]}...")
                    return response_text
        logger.warning(f"No final response received after {event_count} events")
    except Exception as e:
        logger.error(f"Error running job Q&A agent: {e}", exc_info=True)
        raise
    
    logger.info("=== CALL_JOB_QA_AGENT END ===")
    return response_text
